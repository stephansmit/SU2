...
void CLookUpTable_Viscosity::SetViscosity(su2double T, su2double rho){
	...
	UpperJ = Table_Temperature_Stations - 1;
	LowerJ = 0;
	...
	if (LUT_Debug_Mode)
	{
		Check_Interpolated_TRHO_Limits("dMu RHOT");
	}
}
...
void CLookUpTable_Viscosity::Check_Interpolated_TRHO_Limits(string interpolation_case) {
	//Check that the interpolated density and pressure are within LUT limits
	if (rank == MASTER_NODE and LUT_Debug_Mode) {
		if ((Density > Density_Table_Limits[1])
				or (Density < Density_Table_Limits[0])) {
		...
		if ((Temperature > Temperature_Table_Limits[1])
				or (Temperature < Temperature_Table_Limits[0])) {
			cerr << interpolation_case << " Interpolated Temperature out of bounds\n";
		}
	}
}
...
void CLookUpTable_Viscosity::Interpolate_2D_Bilinear_Arbitrary_Skew_Coeff(su2double x,
	...
	if (rank == MASTER_NODE and LUT_Debug_Mode) {
		...
		if (TOP and !BOTTOM) {
			OUT_OF_BOUNDS = true;
			//Check if the point is also outside the LUT
			if (LowerJ == (Table_Temperature_Stations - 2)) {
				cerr << grid_var << ' ' << LowerI << ", " << LowerJ
		...
		for (int var = var_scanned; var < N_PARAM + 1; var++) {
			...
			if (found < 10) {
				...
				if (var == 1) {
					...
					for (int i = 0; i < Table_Density_Stations; i++) {
						for (int j = 0; j < Table_Temperature_Stations; j++) {
					...
					su2double* vP = new su2double[Table_Temperature_Stations];
					var_steps = 10;

					//Each line contains at most 10 pressure values
					for (int k = 0; k < ceil(float(Table_Temperature_Stations) / 10.0);
							k++) {

						getline(table, line);
						istringstream inP(line);
						//Check if line contains less than 10 values
						if ((Table_Temperature_Stations - k * 10) < 10) {
							var_steps = (Table_Temperature_Stations- k * 10);
						}
				...
				else if (x != Table_Density_Stations && y != Table_Temperature_Stations) {
					if (rank == MASTER_NODE) {
						cerr
...
void CLookUpTable_Viscosity::CFX_Import_Table_By_Number(ifstream *tab,
	...
	if (skip_prho) {
		...
		for (int k = 0; k < ceil(float(Table_Temperature_Stations) / 10.0); k++)
	...
	for (int j = 0; j < Table_Temperature_Stations; j++) {
		for (int i = 0; i < Table_Density_Stations; i++) {
			if ((j * Table_Density_Stations + i) % 10 == 0) {
				...
				istringstream in(line);
				var_steps = 10;
				if (((Table_Density_Stations * Table_Temperature_Stations)
						- (j * Table_Density_Stations + i)) < 10)
					var_steps = ((Table_Density_Stations * Table_Temperature_Stations)
...
void CLookUpTable_Viscosity::LookUpTable_Load_DAT(std::string filename) {
	...
	istringstream in(line);
//	in >> Table_Density_Stations >> Table_Temperature_Stations;
	in >> value >> value >> value >> Table_Density_Stations >> value >> value >> value >> Table_Temperature_Stations;
...
void CLookUpTable_Viscosity::LookUpTable_Malloc() {
	...
	for (int i = 0; i < Table_Density_Stations; i++) {
		ThermoTables_Density[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_Pressure[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_Temperature[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_Mu[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_dmudrho_T[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_dmudT_rho[i] = new su2double[Table_Temperature_Stations];
	}
...
void CLookUpTable_Conductivity::SetConductivity(su2double T, su2double rho, su2double mu,
	...
	//Check that the interpolated density and pressure are within LUT limits
	{
		Check_Interpolated_TRHO_Limits("Kt RHOT");
	}
...
void CLookUpTable_Conductivity::SetDerConductivity(su2double T, su2double rho, su2double mu,
	...
	//Check that the interpolated density and pressure are within LUT limits
	{
		Check_Interpolated_TRHO_Limits("dKt RHOT");
	}
}
...
void CLookUpTable_Conductivity::Check_Interpolated_TRHO_Limits(string interpolation_case) {
	//Check that the interpolated density and pressure are within LUT limits
	if (rank == MASTER_NODE and LUT_Debug_Mode) {
		if ((Density > Density_Table_Limits[1])
				or (Density < Density_Table_Limits[0])) {
...
void CLookUpTable_Conductivity::LookUpTable_Malloc() {
	...
	for (int i = 0; i < Table_Density_Stations; i++) {
		...
		ThermoTables_Pressure[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_Temperature[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_Kt[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_dktdrho_T[i] = new su2double[Table_Temperature_Stations];
		ThermoTables_dktdT_rho[i] = new su2double[Table_Temperature_Stations];
